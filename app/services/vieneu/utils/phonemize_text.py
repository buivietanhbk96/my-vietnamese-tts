...
def load_phoneme_dict(path=PHONEME_DICT_PATH):
    ...
    try:
        ...
        logger.warning(f"Phoneme dictionary not found at {path}")
        return {}
...
try:
    ...
    print(f"eSpeak Initialization warning (falling back to dictionary only): {e}")

try:
    phoneme_dict = load_phoneme_dict()
except Exception as e:
    print(f"Phoneme dict loading error: {e}")
